document.addEventListener('DOMContentLoaded', () => {
    ...
    const wsUrl = `ws://${window.location.host}/PBL4-Chess.com/game/${gameId}`;
    ...
    websocket.onmessage = (event) => {
        ...
        if (data.type === 'MOVE_SUCCESS') {
            ...
            if (!startSquare || !endSquare) {
                console.error("Không tìm thấy ô cờ trên bàn cờ!");
                return;
            }

            const pieceImage = startSquare.querySelector('img');

            // Lấy quân cờ ở ô đích (nếu có)
            const capturedPiece = endSquare.querySelector('img');
            if (capturedPiece) {
                // Chỉ xóa quân cờ bị ăn
                capturedPiece.remove();
            }

            if (pieceImage) {
                // Di chuyển quân cờ từ ô bắt đầu sang ô kết thúc
